...
func validateEnvironmentRequirements(result NeedfileResult, envName string) error {
...
func getRuntimeEnvironment(result NeedfileResult, envName, projectDir, projectName string, ctx EvalContext) (environ.RuntimeEnvironment, error) {
	envs := GetEnvironments(result)
	if len(envs) == 0 {
...
// needfileDir is the directory containing the Needfile (for resolving relative paths).
// ctx resolves any interpolation in the environment's .source: path.
// Returns exit code.
func checkEnvironment(result NeedfileResult, envName, needfileDir string, verbose, showInstall bool, ctx EvalContext) int {
	envs := GetEnvironments(result)
	...
	if !isBare {
		...
		if *selectedEnv.Runtime == ast.RuntimeDocker || *selectedEnv.Runtime == ast.RuntimePodman {
			return checkContainerEnvironment(selectedEnv, needfileDir, verbose, showInstall, ctx)
		}

		// Devcontainer environment
		if *selectedEnv.Runtime == ast.RuntimeDevcontainer {
			return checkDevcontainerEnvironment(selectedEnv, needfileDir, verbose, showInstall, ctx)
		}

		// Nix environment
		if *selectedEnv.Runtime == ast.RuntimeNix {
			return checkNixEnvironment(selectedEnv, needfileDir, verbose, showInstall, ctx)
		}

		// Lima environment
		if *selectedEnv.Runtime == ast.RuntimeLima {
			return checkLimaEnvironment(selectedEnv, needfileDir, verbose, showInstall, ctx)
		}
...
func listEnvironments(result NeedfileResult, ctx EvalContext) int {
	envs := GetEnvironments(result)
...
func checkContainerEnvironment(env *ast.Environment, needfileDir string, verbose, showInstall bool, ctx EvalContext) int {
	...
	result, err := detector.DetectDockerfile(env, needfileDir, evalContextOrEmpty(ctx))
	if err != nil {
		fmt.Printf("✗ Source: %s\n", err)
		return exitEnvError
	}
	...
	projectName := filepath.Base(needfileDir)
	envName := ""
	if env.Name != nil {
		envName = *env.Name
	}
...
func checkDevcontainerEnvironment(env *ast.Environment, needfileDir string, verbose, showInstall bool, ctx EvalContext) int {
	detector := environ.NewDevcontainerDetector()
	runner := environ.NewDevcontainerRunner(needfileDir)
	...
	if env.Source != nil {
		...
		configPath = filepath.Join(needfileDir, sourcePath)
		...
		result, err := detector.DetectConfig(needfileDir)
		if err != nil {
...
func checkNixEnvironment(env *ast.Environment, needfileDir string, verbose, showInstall bool, ctx EvalContext) int {
	detector := environ.NewNixDetector()
	runner := environ.NewNixRunner(needfileDir)
	...
	result, err := detector.DetectConfig(needfileDir, env.Source, evalContextOrEmpty(ctx))
	if err != nil {
		fmt.Printf("✗ Configuration: error detecting config: %v\n", err)
		return exitEnvError
	}
...
func checkLimaEnvironment(env *ast.Environment, needfileDir string, verbose, showInstall bool, ctx EvalContext) int {
	...
	runner := environ.NewLimaRunner(needfileDir, vmName)
