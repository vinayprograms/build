...
// needfileResultAdapter wraps the result of ParseNeedfile.
type needfileResultAdapter struct {
	statements []ast.Statement
	errors     *parser.ParseErrors
}

func (br needfileResultAdapter) Statements() []Statement {
...
func (br needfileResultAdapter) ErrorCount() int {
	return len(br.errors.Errors)
}

func (br needfileResultAdapter) GetError(i int) ParseError {
...
func (br needfileResultAdapter) HasErrors() bool {
	return br.errors.HasErrors()
}

func (br needfileResultAdapter) AllErrors() string {
	return br.errors.Error()
}

// needfileParserAdapter wraps parser.Parser to implement NeedfileParser.
type needfileParserAdapter struct {
	p *parser.Parser
}

func (bp *needfileParserAdapter) ParseNeedfile() NeedfileResult {
	stmts, errs := bp.p.ParseNeedfile()
	return needfileResultAdapter{
		statements: stmts,
		errors:     errs,
	}
}

// NewNeedfileParser creates a NeedfileParser from a Parser.
func NewNeedfileParser(p Parser) NeedfileParser {
	if pa, ok := p.(*parserAdapter); ok {
		return &needfileParserAdapter{p: pa.p}
	}
	panic("NewNeedfileParser requires a Parser created by NewParser")
}

// GetASTStatements returns the raw AST statements from a NeedfileResult.
// This is useful for operations that need direct access to ast types.
func GetASTStatements(result NeedfileResult) []ast.Statement {
	if br, ok := result.(needfileResultAdapter); ok {
...
func GetEnvironments(result NeedfileResult) []*ast.Environment {
