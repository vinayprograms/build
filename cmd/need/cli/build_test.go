...
`
	...
	exitCode := Run([]string{"-f", needfile, "hello"})
...
`
	...
	exitCode := Run([]string{"-f", needfile, "fail"})
...
`
	...
	exitCode := Run([]string{"-f", needfile, "-n", "build"})
	if exitCode != exitSuccess {
		t.Errorf("exit code = %d, want %d", exitCode, exitSuccess)
	}
...
`
	...
	exitCode := Run([]string{"-f", needfile, "first", "second", "third"})
	if exitCode != exitSuccess {
		t.Errorf("exit code = %d, want %d", exitCode, exitSuccess)
	}
}
...
`
	...
	exitCode := Run([]string{"-f", needfile, "check"})
	if exitCode != exitSuccess {
		t.Errorf("exit code = %d, want %d", exitCode, exitSuccess)
	}
}
