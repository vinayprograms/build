...
func Run(args []string) int {
	...
	needfile := f.file
	if needfile == "" {
		needfile = findNeedfile()
	}

	if needfile == "" {
		fmt.Fprintln(os.Stderr, "error: no Needfile found")
		return exitParseError
	}

	// Debug mode: dump lexer tokens
	if f.debugLex {
		return debugLexer(needfile)
	}

	// Debug mode: dump parser scope validation
	if f.debugParse {
		return debugParser(needfile)
	}

	// Debug mode: dump variable parsing
	if f.debugVar {
		return debugVariables(needfile)
	}

	// Debug mode: dump target parsing
	if f.debugTarget {
		return debugTargets(needfile)
	}

	// Debug mode: dump recipe parsing
	if f.debugRecipe {
		return debugRecipes(needfile)
	}

	// Debug mode: dump environment parsing
	if f.debugEnv {
		return debugEnvironments(needfile)
	}

	// Debug mode: dump conditional parsing
	if f.debugCond {
		return debugConditionals(needfile)
	}

	// Debug mode: dump include parsing
	if f.debugIncl {
		return debugIncludes(needfile)
	}

	// Debug mode: dump full AST with error recovery
	if f.debugAST {
		return debugAST(needfile)
	}

	// Debug mode: dump semantic analysis (symbol table)
	if f.debugSemantic {
		return debugSemantic(needfile)
	}

	// Debug mode: dump variable evaluation
	if f.debugEval {
		return debugEval(needfile)
	}

	// Debug mode: dump build planning (target matching)
	if f.debugPlan {
		return debugPlan(needfile)
	}

	// Parse the needfile (with caching)
	result, content, err := ParseNeedfileWithCache(needfile)
	if err != nil {
	...
	// is displayed the same way it will be used - a Needfile that only
	// defines variables used by environments still needs no semantic
	// analysis to list them, so this runs before that phase.
	if f.listEnv {
		listEnvCtx := EvaluateVariables(result, filepath.Dir(needfile))
	...
	// Evaluate variables
	needfileDir := filepath.Dir(needfile)
	evalResult := EvaluateVariables(result, needfileDir)
	...
	// build.
	if f.checkEnv {
		return checkEnvironment(result, f.env, needfileDir, f.verbose, f.showInstall, evalResult.Context())
	}

	// Show what was parsed (verbose mode) - basic info before planning
	if f.verbose {
		fmt.Printf("Needfile: %s\n", needfile)
	...
	absNeedfileDir, err := filepath.Abs(needfileDir)
	if err != nil {
		fmt.Fprintf(os.Stderr, "error: cannot resolve needfile path: %v\n", err)
		return exitParseError
	}
	projectName := filepath.Base(absNeedfileDir)
	runtimeEnv, err := getRuntimeEnvironment(result, f.env, absNeedfileDir, projectName, ctx)
	if err != nil {
		fmt.Fprintf(os.Stderr, "error: %v\n", err)
		return exitEnvError
	}
...
func parseFlags(args []string) (*flags, []string, error) {
	...
	fs := flag.NewFlagSet("need", flag.ContinueOnError)
	fs.SetOutput(os.Stderr)

	fs.StringVar(&f.file, "file", "", "Use alternate Needfile")
	fs.StringVar(&f.file, "f", "", "Use alternate Needfile (shorthand)")
	...
	// NEED_ENV selects an environment when --env is not given (lower precedence).
	if f.env == "" {
		f.env = os.Getenv("NEED_ENV")
	}
...
func printUsage() {
	fmt.Print(`Usage: need [options] [targets...]
...
Examples:
  need                     Run the default target
  need @test               Run phony target
  need -n                  Dry run (show what would execute)
  need -v                  Verbose output (show staleness checks)
  need -f other.need       Use alternate file
  need -j4                 Run with 4 parallel jobs
  need @clean @build       Run multiple targets in order

Environment Commands:
  need --list-env          List all defined environments
  need --check-env         Check if requirements are satisfied
  need --check-env -e ci   Check named environment "ci"
  need --check-env --show-install
...
For more information, see: https://github.com/vinayprograms/need
`)
}
...
func findNeedfile() string {
	candidates := []string{"Needfile", "needfile"}
