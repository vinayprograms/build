...
// TestParser_IsTargetLine tests the line classification helper.
func TestParser_IsTargetLine(t *testing.T) {
	tests := []struct {
		name     string
		input    string
		isTarget bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := p.IsTargetLine()
			if got != tt.isTarget {
				t.Errorf("IsTargetLine() = %v, want %v", got, tt.isTarget)
			}
		})
	}
}
