# Needfile Language Specification v1.0

## Overview

Needfile is a language for describing build targets, their dependencies, and recipes to produce them. It preserves Make's powerful file-based dependency model while eliminating its syntactic quirks.
...
Needfile uses indentation to define structure. Unlike Make, **tabs are not required**.
...
- Be consistent within a single Needfile
...
| `.include:` | Include another Needfile | — |
...
| `NEED_ENV` variable set | Use that environment (lower precedence than `--env`) |

### Shared Environments

Use `.include:` to share environments across Needfiles:
...
| Command | Description |
|---------|-------------|
| `need` | Run default target with default environment |
| `need target` | Build specific target |
| `need @phony` | Run phony target |
...
| `--file path` | `-f path` | Use alternate Needfile |
...
**Use alternate Needfile:**

```bash
$ need -f ./other/Needfile
```
...
Sandbox kept running. To enter:
  docker exec -it need-ci-a1b2c3 /bin/bash

To stop:
  docker stop need-ci-a1b2c3
```
...
This section provides guidance for implementing the `need` tool.
...
1. **Parse phase**: Read Needfile, resolve includes, build AST
...
This concludes the Needfile language specification v1.0.
